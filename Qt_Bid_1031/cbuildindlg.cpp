...
void CBuildinDlg:: doPushBtn()
{
    ...
    if(btn == errorPushBtn)
    {
        errorBox->showMessage(QStringLiteral("錯誤訊息實例xx。"));
        errorBox->showMessage(QStringLiteral("錯誤訊息實例yy。"));
        errorBox->showMessage(QStringLiteral("錯誤訊息實例zz。"));
        errorBox->show();
    }
    if(btn ==filePushBtn)
    {
    ...
    if(btn == textColorPushBtn)
    {
        QPalette palette=displayTextEdit->palette();
        const QColor& color=
            QColorDialog::getColor(palette.color(QPalette::Text),this,tr("字體顏色"));
        if(color.isValid())
        {
